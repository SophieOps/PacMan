...
Pour ce qui est des fichiers "package-info.java", il sont a nouveau inutule lors de l'exécution du code, mais permettent de contenir les commentaires contenant l'information relative au package en vue de la création de la javadoc. Ces fichiers sont donc à conserver (et à complêter dans certains cas).
...
\begin{itemize}
\item La classe MainUI a beaucoup de client (c'est-à-dire beaucoup d'autres classes qui accèdent aux données publiques de cette classes) dont les principaux sont "PacmanKeyListener" et "IGameInteractor".
\item La classe "Player" a beaucoup de client et aucun fournisseur\footnote{>provider} dont les principaux sont "Game" et "BoardView".
\item La classe "Sprite" a beaucoup de client et aucun fournisseur dont les principaux sont "Game", "Board" et "Tile".
\item Les classes tel que "Ghost", "Controller", "Wall", "ImageLoader", "Animator", "MapParser", "IGameFactory", "FactoryException", "IPacmanInteraction" et "Disposable" n'ont ni client ni fournisseur.
\end{itemize}
